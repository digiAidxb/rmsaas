...
function clearAllData() {
    ...
    const firstConfirm = confirm(
        '⚠️ WARNING: Clear All Restaurant Data?\n\n' +
    ...
    const confirmation = prompt(
        '🔐 SAFETY CHECK: Type "DELETE ALL DATA" to confirm:\n\n' +
    ...
    if (confirmation !== 'DELETE ALL DATA') {
        if (confirmation !== null) {
            alert('❌ Confirmation text does not match. Data clearing cancelled for your safety.');
        }
        return;
    }
    ...
    const finalConfirm = confirm(
        ...
        '• Remove ALL import records\n' +
        '• Reset your restaurant database\n\n' +
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                alert(
                    ...
                    `📊 Summary:\n` +
                    `• Tables cleared: ${clearedTables.length}\n` +
                    `• Records deleted: ${totalRecords.toLocaleString()}\n` +
                    `• Processing time: ${(data.execution_time || 0).toFixed(2)}s\n\n` +
            ...
            alert('❌ Error clearing data: ' + (data.message || 'Unknown error occurred'));
        }
    })
    .catch(error => {
